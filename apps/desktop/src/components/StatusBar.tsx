...
function PermissionModeSegment({ onShowSettings: _onShowSettings }: { onShowSettings?: (tab?: string) => void }) {
  const { settings, updateSettings } = useSettings();
  const [open, setOpen] = useState(false);
  const ref = useRef<HTMLDivElement>(null);
...
function ConnectionIndicator({ isStreaming }: { isStreaming: boolean }) {

              ...
              <div className="flex flex-1 flex-col min-w-0 min-h-0">
                ...
                {activeSessionId || pendingMessage ? (
                  <ChatPage key={activeSessionId ?? 'new-chat'} sessionId={activeSessionId ?? pendingWelcomeSessionId} onCreateSession={handleNewChat}
                    ...
                    onToggleSidebar={() => setSidebarOpen(o => !o)} onOpenSettings={handleOpenSettings} onOpenSessions={() => { setActiveView('chat'); setSidebarOpen(true); }} onOpenPullRequests={() => setActiveView('teams')}
...
function App() {
  ...
  return (
    <ErrorBoundary>
      <SettingsProvider>
        <AuthGate>{(auth) => <AppLayout email={auth.email} plan={auth.plan} />}</AuthGate>

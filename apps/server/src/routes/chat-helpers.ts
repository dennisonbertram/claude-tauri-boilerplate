...
import { z } from 'zod';
import { PERMISSION_MODES, PROVIDER_CONFIG_FIELD_KEYS, PROVIDER_TYPES } from '@claude-tauri/shared';
...
import { buildAdditionalDirectoryPathPolicy, buildWorkspaceAttachmentPathPolicy, canonicalizePath, canonicalizeRoots, isPathWithinAnyRoot } from '../utils/paths';

const providerConfigShape = Object.fromEntries(PROVIDER_CONFIG_FIELD_KEYS.map((key) => [key, z.string().optional()])) as Record<ProviderConfigFieldKey, z.ZodOptional<z.ZodString>>;
const chatMessageSchema = z.object({ role: z.enum(['user', 'assistant']), content: z.string().optional(), parts: z.array(z.object({ type: z.string(), text: z.string().optional() })).optional() });

export const chatRequestSchema = z.object({ messages: z.array(chatMessageSchema).min(1), sessionId: z.string().nullish(), profileId: z.string().uuid().optional(), provider: z.enum(PROVIDER_TYPES).optional(), providerConfig: z.object(providerConfigShape).optional(), runtimeEnv: z.record(z.string(), z.string()).optional(), model: z.string().optional(), effort: z.enum(['low', 'medium', 'high', 'max']).optional(), thinkingBudgetTokens: z.number().int().min(1024).max(32000).optional(), permissionMode: z.enum(PERMISSION_MODES).optional(), workspaceId: z.string().optional(), additionalDirectories: z.array(z.string().min(1)).optional(), systemPrompt: z.string().optional(), linearIssue: z.object({ id: z.string(), title: z.string(), summary: z.string().optional(), url: z.string().optional() }).optional(), attachments: z.array(z.string().min(1)).optional() });

export const linearIssueSchema = z.object({ id: z.string().min(1, 'issue id is required'), title: z.string().min(1, 'issue title is required'), summary: z.string().optional(), url: z.string().url().optional() });
...
export function logChatDebug(message: string, fields?: Record<string, unknown>) { if (!CHAT_DEBUG_LOGS_ENABLED) return; logChat('info', message, fields); }

export function countConfiguredProviderValues(config: ChatRequest['providerConfig']): number { return Object.values(config ?? {}).filter((value) => Boolean(value?.trim())).length; }
...
export async function resolveWorkspaceAttachments(attachments: string[], workspaceCwd: string, allowedRoots: string[], errorMessage: string): Promise<string[]> { return Promise.all(attachments.map((a) => sanitizeAttachmentReference(a, workspaceCwd, allowedRoots, errorMessage))); }
...
export async function normalizeWorkspaceAdditionalDirectories(input: string[], workspaceRoot: string, allowedRoots: string[], errorMessage: string): Promise<string[]> {
  ...
  for (const rawEntry of input) { const trimmed = rawEntry.trim(); if (!trimmed) continue; const resolved = isAbsolute(trimmed) ? trimmed : resolve(workspaceRoot, trimmed); const canonical = await canonicalizePath(resolved); if (!isPathWithinAnyRoot(canonical, allowedRoots)) throw new Error(errorMessage); normalized.add(canonical); }
  return [...normalized];
}
...
function resolveInstructionPath(overridePath: string | undefined, fallbackPath: string): string { const trimmed = overridePath?.trim(); return trimmed ? trimmed : fallbackPath; }
async function readInstructionFile(filePath: string): Promise<{ path: string; exists: boolean; content: string }> { try { const file = Bun.file(filePath); const exists = await file.exists(); if (!exists) return { path: filePath, exists: false, content: '' }; return { path: filePath, exists: true, content: await file.text() }; } catch { return { path: filePath, exists: false, content: '' }; } }
...
export async function buildStartupPrompt(workspaceRoot?: string, systemPrompt?: string): Promise<string> {
  ...
  const [globalFile, userFile, workspaceManagedFile, workspaceFile] = await Promise.all([readInstructionFile(globalPath), readInstructionFile(userPath), readInstructionFile(join(resolvedWorkspaceRoot, '.claude', 'CLAUDE.md')), readInstructionFile(join(resolvedWorkspaceRoot, 'CLAUDE.md'))]);
  ...
  const instructionBlocks = [globalFile.exists && globalFile.content.trim() ? `[Global Instruction]\n${globalFile.content.trim()}` : undefined, userFile.exists && userFile.content.trim() ? `[User Instruction]\n${userFile.content.trim()}` : undefined, preferredWorkspaceFile ? `[${preferredWorkspaceFile.label} Instruction]\n${preferredWorkspaceFile.content}` : undefined, systemPrompt?.trim() ? `[System Prompt]\n${systemPrompt.trim()}` : undefined].filter((item): item is string => item !== undefined);
...
export function classifyStreamError(err: unknown): StreamError {
  ...
  const status = (err as any).status; const code = (err as any).code;

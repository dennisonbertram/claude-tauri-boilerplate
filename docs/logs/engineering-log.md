...
**Type**: Bug Fix
**Impact**: Medium
**Description**: Completed GitHub issue `#115` in the issue worktree by replacing the earlier toast-only memory nudges with a real repo-memory update flow. Added repo-scoped `reviewMemory` and `mergeMemory` workflow prompts stored through the existing `/api/memory` files, wired review-feedback and post-merge actions to queue a pending `MEMORY.md` draft, and updated the Memory tab to consume that draft directly into the editor so saving persists through the normal memory routes and future sessions keep using the same repository memory files.

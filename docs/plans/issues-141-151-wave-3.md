...
- [ ] Live issue details are pulled from GitHub and reflected in the execution plan.
- [ ] A new integration branch is created from `codex/wave2-cleanups`.
- [ ] Up to six isolated issue worktrees are created for the first batch.
- [ ] Every worked issue gets either:
  - [ ] failing test first, then fix, then passing regression coverage, or
  - [ ] regression coverage only when the fix already exists on the integration line.
- [ ] `docs/plans/` contains wave-level handoff status and manual browser-control verification notes for every worked issue.
- [ ] `docs/plans/INDEX.md` stays current.
...
- [ ] All issue branches are merged into a single wave integration branch and validated there.
- [ ] A cleanup/review pass resolves red tests and integration conflicts before any merge to `main`.
...
- [ ] `#141` regression coverage / stale session recovery validation
- [ ] `#142` regression coverage / export filename sanitization
- [ ] `#143` regression coverage / Add Project touched-state validation
- [ ] `#144` regression coverage / empty-session message clearing
- [ ] `#145` regression coverage / right-click context menu
- [ ] `#146` regression coverage / visible compact feedback

## Second Batch

- [ ] `#147` regression coverage / SSE idle-timeout protection
- [ ] `#148` regression coverage / narrow-width protection
- [ ] `#149` regression coverage / forked session history replay
- [ ] `#150` implement shared `parseToolInput` utility and migrate callers
- [ ] `#151` implement output sanitization utilities and migrate renderers

## Integration and Review

- [ ] Merge first-batch branches into `codex/wave3-merge`
- [ ] Run targeted frontend/server tests on the integrated branch
- [ ] Merge second-batch branches into `codex/wave3-merge`
- [ ] Run cleanup review for conflicts, red tests, and integration regressions
- [ ] Validate browser/server behavior from the integration branch
